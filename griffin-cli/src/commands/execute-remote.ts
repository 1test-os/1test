...
export async function executeExecuteRemote(checkName: string): Promise<void> {
  ...
  if (!runnerHost) {
    console.error(
      "ERROR: No runner host configured. Run: griffin configure-runner-host <host>",
    );
    process.exit(1);
  }

  console.log(
    "WARNING: Your local checks will be deployed and executed remotely.",
  );
  ...
  rl.question("", async (answer) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${runnerHost}/api/tests/${checkName}/execute`,
      );
