...
export const AgentConfigSchema = Type.Object({
    ...
    hub: Type.Object({
        ...
        apiKey: Type.Optional(Type.String({ description: "Optional API key for authentication" })),
    }),
    // Queue consumer configuration
    queue: Type.Object({
        backend: Type.Union([Type.Literal("postgres"), Type.Literal("sqs"), Type.Literal("redis")], { default: "postgres" }),
...
export function loadAgentConfigFromEnv() {
    ...
    // Queue backend
    const queueBackend = (process.env.QUEUE_BACKEND || "postgres");
    // Validate queue backend
    if (!["postgres", "sqs", "redis"].includes(queueBackend)) {
        throw new Error(`Invalid QUEUE_BACKEND: ${queueBackend}. Must be one of: postgres, sqs, redis`);
    }
    // Determine queue connection string
    let queueConnectionString;
    if (queueBackend === "postgres") {
        queueConnectionString =
            process.env.QUEUE_CONNECTION_STRING || process.env.POSTGRESQL_URL;
        if (!queueConnectionString) {
            throw new Error("QUEUE_CONNECTION_STRING or POSTGRESQL_URL is required when QUEUE_BACKEND=postgres");
        }
    }
        ...
        if (!queueConnectionString) {
            throw new Error("QUEUE_CONNECTION_STRING (SQS queue URL) is required when QUEUE_BACKEND=sqs");
        }
    }
        ...
        if (!queueConnectionString) {
            throw new Error("QUEUE_CONNECTION_STRING or REDIS_URL is required when QUEUE_BACKEND=redis");
        }
    }
