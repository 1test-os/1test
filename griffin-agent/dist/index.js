...
import { AxiosAdapter, SecretProviderRegistry, EnvSecretProvider, } from "griffin-plan-executor";
/**
...
async function main() {
    ...
    if (config.queue.backend === "postgres") {
        ...
        queueConsumer = new PostgresQueueConsumer(config.queue.connectionString, config.queue.queueName);
    }
    else {
    ...
    // Create worker
    const worker = new WorkerService(config.agent.location, queueConsumer, agentApi, runsApi, configApi, {
    ...
    if (config.heartbeat.enabled) {
        console.log(`Starting heartbeat loop (interval: ${config.heartbeat.interval}s)`);
