 ...
 */
export class PostgresQueueConsumer {
    connectionString;
    queueName;
    pool = null;
    constructor(connectionString, queueName = "plan-executions") {
    ...
    async fail(jobId, error, retry = true) {
        ...
        const result = await this.pool.query(`SELECT attempts, max_attempts as "maxAttempts" FROM jobs WHERE id = $1`, [jobId]);
        if (result.rows.length === 0) {
...
//# sourceMappingURL=postgres-consumer.js.map